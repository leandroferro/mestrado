...
public abstract class CommandSerializer {
	...
	private static final CommandSerializer serializer = new CommandSerializer() {
		
		@Override
		String _serialize(Command command) {
	...
	public static String serialize(Command command) {
		return serializer._serialize(command);
	}
	
	abstract String _serialize(Command command);
