...
% O Algoritmo \ref{alg:transferencia_valores_contas_distribuidas} utiliza a primitiva de transação criada na seção anterior para criar duas transações isoladas, uma em cada gerenciador de recurso, e efetuar a transferência dos valores. O comportamento das funções $Ler$ e $Escrever$ é o mesmo apresentado no Algoritmo \ref{alg:transferencia_valores_transacao} (a ocorrência de um erro aborta a transação e termina o programa). Se ocorrer um problema ao efetivar a transação $T_D$ (por exemplo, se a conexão de rede com a máquina de destino for perdida), a transação $T_O$ já terá sido efetivada, e assim os dados ficarão incorretos.
...
%     $D \gets \text{Recurso referente à conta de destino}$\;
%     $V \gets \text{Valor a ser transferido}$\;
%     $T_O \gets IniciarTransacao(G_O)$\;
%     $T_D \gets IniciarTransacao(G_D)$\;
%     $V_O \gets Ler(T_O, O)$\;
%     \eSe{$V_O >= V$}
%     {
%         $V_D \gets Ler(T_D, D)$\;
%         $Escrever(T_O, O, V_O - V)$\;
%         $Escrever(T_D, D, V_D + V)$\;
%         $Efetivar(T_O)$\;
%         $Efetivar(T_D)$\;
%     }
%     {
