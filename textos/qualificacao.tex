...
A execução da transação \(I_t\) é efetuada de maneira distribuída entre o coordenador da transação e o participante. O comportamento do coordenador é descrito pelo Algoritmo \ref{alg:mini_coordenador}. Diferentemente do \emph{2PC}, o protocolo de execução das minitransações não grava nenhum registro no \emph{log} do coordendaor. O coordenador recebe a requisição de uma transação no formato $(I_t, C, L, E)$ e envia uma minitransação no formato $M = (I_t, C_j, L_j, E_j)$ para cada participante \(P_j\) da transação, como demonstramos no Algoritmo \ref{alg:mini_coordenador}. $I_t$ é o identificador da transação, devendo ser único em todo o sistema. $C$ é o conjunto de itens de comparação, $L$ é o conjunto de itens a serem lidos e $E$ é o conjunto de dados a serem atualizados ou inseridos. $L_j, Id[C_j]\text{ e } Id[E_j] \in C$ são os conjuntos de identificadores da transação que estão armazenados em $P_J$.

\begin{algorithm}
\caption{Coordenador - recebe uma transação no formato $(I_t, C, L, E)$}
...
\ForAll{$p_j \in Participantes$}
    ...
    \EndIf
\EndFor
% \Comment{Segunda Fase - Notificação}
\If{$d \neq ABORTAR$}
