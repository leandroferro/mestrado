...
Como descrito anteriormente na seção \ref{sec:transacoes}, a maneira usual de efetuar controle de concorrência é através do uso de travas associadas aos dados acessados. Em um sistema não distribuído, o controle de alocação e liberação destas travas fica a cargo de um gerenciador de travas, um único componente que centraliza e sabe sobre todas as travas ativas no banco. Em um sistema distribuído, este gerenciamento de travas pode ser feito de três maneiras:
...
\label{sec:2pc}
A idéia deste protocolo é efetuar uma rodada de votação, para coletar os votos das máquinas participantes da transação. Cada participante pode votar SIM ou NÃO para respectivamente efetivar ou abortar a transação. Após essa coleta os votos são avaliados para saber se alguma máquina votou NÃO. Caso haja no mínimo um voto NÃO, a transação será abortada, e todos os componentes serão notificados para abortar a transação. Se todos os votos forem SIM, então a transação será efetivada, e os participantes serão notificados também. 

Uma máquina específica, chamada coordenador, é responsável por iniciar a rodada de votação, coletar os votos, decidir sobre o resultado da votação e notificar os participantes sobre este resultado. Cada máquina deve manter um \emph{log} para registrar as ações de uma subtransação, mas não há um log global. A cada mensagem enviada, um registro é feito no log da máquina que enviou a mensagem para auxiliar na recuperação em caso de falhas.

O protocolo de efetivação executa os seguintes passos:
...
\State $R \gets \{\}$
\For{$\imath \gets$ índice de cada participante}
...
\EndFor
\If{$\forall v \in Voto(R) : v = SIM$ e $Processar(Dado(R)) = OK$}
