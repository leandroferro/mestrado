...
A seção \ref{sec:tolerancia_a_falhas} cobre os principais conceitos relacionados a transações e cria um vocabulário que será utilizado por todo o texto deste trabalho.
...
%A garantia da atomicidade envolve procedimentos que vão além de efetivar ou cancelar a transação no momento em que ela é finalizada.  

%Por exemplo, em geral escritas de dados em discos rígidos são postergadas para melhorar a performance. Os dados são agrupados e mantidos temporariamente na memória principal, em um \emph{buffer}, para depois serem enviados realmente para a gravação física no disco.

%Considerando a execução de um programa implementando o algoritmo \ref{alg:transferencia_transacao_2}, que armazena tanto o estado quanto o registro de atividades no disco, podemos imaginar um cenário que ocorra o seguinte :

%\begin{itemize}
%	\item As operações $IniciarTransacao$, $X = X - V$ e $Y = Y + V$ já foram executadas e registradas, e o registro foi armazenado no disco.
%	\item A aplicação executa $EfetivarTransacao$, que é registrada no \emph{log}, mas esse registro não é gravado no disco ainda.
%	\item A aplicação continua sua execução, assumindo que as modificações.
%\end{itemize}
